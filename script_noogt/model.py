...
class Model(nn.Module):
    ...
    def forward(self, x, adj):  			# x.shape = (seq_len, FEATURE_DIM); adj.shape = (seq_len, seq_len)
        ...
        protein_features = x[:,:, TRADITION_SIZE:]
        assert protein_features.size(2) == 1280, f"MLP Wrong: {protein_features.size()}"
        processed_protein = self.mlp(protein_features)        
        x = torch.cat([manual_features, processed_protein], dim=2)
        assert x.size(2) == TRADITION_SIZE+MLP_OUTPUT_DIM, f"Wrong: {x.size()}"
